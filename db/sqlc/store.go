...
type Store interface {
	...
	UpdateSongTx(ctx context.Context, arg UpdateSongTxParams) (Song, error)
}
...
type UpdateSongTxParams struct {
	ID       int64
	Name     string
	Singer   string
	Image    string
...
func (store *SQLStore) UpdateSongTx(ctx context.Context, arg UpdateSongTxParams) (Song, error) {
	var song Song

	err := store.execTx(ctx, func(q *Queries) error {
		res, err := q.UpdateSong(ctx, UpdateSongParams{
			ID:       arg.ID,
			Name:     arg.Name,
			Image:    arg.Image,
			Singer:   arg.Singer,
		...
		genres, err := q.ListSongsGenres(ctx, res.ID)
		...
		for i := 0; i < len(genres); i++ {
			if !includes(genres[i], arg.Genres) {
				err := q.DeleteSongGenre(ctx, DeleteSongGenreParams{
					GenresID: genres[i],
					SongsID:  res.ID,
				})
...
func includes(element int64, arr []int64) bool {
	for i := 0; i < len(arr); i++ {
		if element == arr[i] {
			return true
		}
